...
public class Doc_AllocationHdr extends Doc
{
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			// BEGIN ogomezislas Reclasificación de Impuesto
			if (invoice != null & payment != null)
			{
				...
				try
				{
					...
					while (rs.next())
					{
						BigDecimal percent = allocationSource.divide(invoice.getGrandTotal(), BigDecimal.ROUND_HALF_DOWN, 4);
						MFactAcct fa = new MFactAcct(getCtx(), rs, fact.get_TrxName());
						MTax tax = new MTax(Env.getCtx(), fa.getC_Tax_ID(), null);
						String columnDebit = invoice.isSOTrx() ? "T_Liability_Acct" : "T_Receivables_Acct";
						String sqlDebitTax = "SELECT C_ValidCombination_ID FROM C_ValidCombination WHERE C_ValidCombination_ID=("
								+ " SELECT " + columnDebit + " FROM C_Tax_Acct WHERE C_Tax_ID=?)";
						int accountDebitId = DB.getSQLValue(tax.get_TrxName(), sqlDebitTax, tax.get_ID());
						MAccount accountDebitTax = MAccount.get(getCtx(), accountDebitId);

						String columnCredit = invoice.isSOTrx() ? "T_Due_Acct" : "T_Credit_Acct";
						String sqlCreditTax = "SELECT C_ValidCombination_ID FROM C_ValidCombination WHERE C_ValidCombination_ID=("
								+ " SELECT " + columnCredit + " FROM C_Tax_Acct WHERE C_Tax_ID=?)";
						int accountCreditId = DB.getSQLValue(tax.get_TrxName(), sqlCreditTax, tax.get_ID());
						MAccount accountCreditTax = MAccount.get(getCtx(), accountCreditId);

						if ((fa.getAccount_ID() == accountCreditTax.getAccount_ID() && fa.getAmtAcctDr().signum() > 0)
						 || (fa.getAccount_ID() == accountDebitTax.getAccount_ID() && fa.getAmtAcctCr().signum() > 0)) {
							FactLine dbTax = fact.createLine(null, accountDebitTax, as.getC_Currency_ID(), fa.getAmtAcctDr().multiply(percent), Env.ZERO);
							dbTax.setAD_Org_ID(fa.getAD_Org_ID());
							dbTax.setC_Tax_ID(fa.getC_Tax_ID());
							dbTax.setC_Project_ID(fa.getC_Project_ID());
							dbTax.setC_Activity_ID(fa.getC_Activity_ID());
							dbTax.saveEx();
							//
							FactLine crTax = fact.createLine(null, accountCreditTax, as.getC_Currency_ID(), Env.ZERO, fa.getAmtAcctDr().multiply(percent));
							crTax.setAD_Org_ID(fa.getAD_Org_ID());
							crTax.setC_Tax_ID(fa.getC_Tax_ID());
							crTax.setC_Project_ID(fa.getC_Project_ID());
							crTax.setC_Activity_ID(fa.getC_Activity_ID());
							crTax.saveEx();
						} else if ((fa.getAccount_ID() == accountCreditTax.getAccount_ID() && fa.getAmtAcctCr().signum() > 0)
								|| (fa.getAccount_ID() == accountDebitTax.getAccount_ID() && fa.getAmtAcctDr().signum() > 0)){
							FactLine dbTax = fact.createLine(null, accountDebitTax, as.getC_Currency_ID(), Env.ZERO , fa.getAmtAcctCr().multiply(percent));
							dbTax.setAD_Org_ID(fa.getAD_Org_ID());
							dbTax.setC_Tax_ID(fa.getC_Tax_ID());
							dbTax.setC_Project_ID(fa.getC_Project_ID());
							dbTax.setC_Activity_ID(fa.getC_Activity_ID());
							dbTax.saveEx();
							//
							FactLine crTax = fact.createLine(null, accountCreditTax, as.getC_Currency_ID(), fa.getAmtAcctCr().multiply(percent) , Env.ZERO);
							crTax.setAD_Org_ID(fa.getAD_Org_ID());
							crTax.setC_Tax_ID(fa.getC_Tax_ID());
							crTax.setC_Project_ID(fa.getC_Project_ID());
							crTax.setC_Activity_ID(fa.getC_Activity_ID());
